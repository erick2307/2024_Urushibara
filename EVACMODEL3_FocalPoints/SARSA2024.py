...
class SARSA:
    def __init__(self, 
                 foldername = "Input",
                 agentsProfileName="population.csv", 
        ...
        self.linksdb = np.loadtxt(linksdbFile, delimiter=',', skiprows=1) 
        ...
        self.popHistPerLink = np.zeros(( self.linksdb.shape[0] , int(max(self.popAtLink_HistParam[:,1]))+2 ))
        # 2020Oct07: Memory for density array and density level at links
        self.denArrPerLink= np.zeros(( self.linksdb.shape[0] , int(max(self.popAtLink_HistParam[:,1]))+2 ))
        self.denLvlArrPerLink= np.zeros(( self.linksdb.shape[0] , int(max(self.popAtLink_HistParam[:,1]))+2 ))
        # 2020Oct07: Memory for velocity array at links
        self.speArrPerLink= np.zeros(( self.linksdb.shape[0] , int(max(self.popAtLink_HistParam[:,1]))+2 ))
        ...
        self.pedProfiles = np.loadtxt(agentsProfileName, delimiter=',', skiprows=1).astype("int")
        ...
        #     firstState = np.zeros(3, dtype = np.int64)
    ...
    def computePedHistDenVelAtLinks(self):
        ...
        for ind in occupLinksIndx:
            ...
            # print("numComp", numComp, ind)
            # print(self.popAtLink_HistParam[ind,:])
            ...
            self.denLvlArrPerLink[ind, :numComp] = np.zeros(numComp)
            indxThreshold1= self.denArrPerLink[ind, :] > 0.5
            self.denLvlArrPerLink[ind, indxThreshold1] = 1
            indxThreshold2= self.denArrPerLink[ind, :] > 3.0
            self.denLvlArrPerLink[ind, indxThreshold2] = 2
        return
    ...
    def computeDensityLevel(self, codeLink, linkWidth = 2.):
        denLvl= np.max(self.denLvlArrPerLink[codeLink, :])
        return denLvl
...
#            print("high density of %.4f at %d" % (density,codeLink))
    ...
    def updateVelocityV2(self, pedIndx):
        # indxNE= self.time < self.pedDB[:,9]
        # print("1",self.pedDB[indxNE,4])
        ...
        if codeLink == -1:
            ...
            xAtLink= np.int64(np.floor( dist / unitL))
            ...
            # print("3",self.pedDB[indxNE,4])
            
        # print(indxNE, pedIndx)
        # print(" ")
        return
    ...
    def initEvacuationAtTime(self):
        ...
        if len(indxPed) != 0:
            for i in indxPed:
                ...
                node0 = int(self.pedDB[i,8]) # current node
                # print("\n*****")
                # print(indxPed, i, node0, int(self.transNodedb[node0,1]))
                indxTgt = np.random.choice( int(self.transNodedb[node0,1]) ) # random choise for the next node
                nodeTgt = self.transNodedb[node0, 2+indxTgt] # next number node
                link = self.transLinkdb[node0, 2+indxTgt] # next link code
                self.pedDB[i,7] = nodeTgt
    ...
    def updateTarget(self, pedIndx, ifOptChoice = False):
        ...
        # Otherwise, we assign new target node:
            ...
            if nodeTgt == node0:
                ...
                expeStatAndVal = np.array([stateIndx, indxTgt, self.time], dtype=np.int64)
    ...
    def updateTargetShortestPath(self, pedIndx):
        if self.pedDB[pedIndx, 10]:
            ...
            # print("nodesLinked",nodesLinked, nodeTgt)
    ...
    def computeAction_Value_Policy(self):
        ...
        # Get policy
        nodeState = self.stateMat[ : , 0 ].astype(np.int64)
    ...
    def setFigureCanvas(self, sizeCanvas= (8,5)):
        self.fig, self.ax = plt.subplots(figsize=sizeCanvas)
    ...
    def getSnapshotV2(self, foldername = "Input", ifNodeAround= False, nodeAroundCode= 1070):
        self.p2.remove()
        indx = np.where(self.pedDB[:,9] <= self.time)[0]
        # 2020Oct07: trying to place color velocity
        # self.p2, = self.ax.plot(self.pedDB[indx,0], self.pedDB[indx,1], 'ro', ms=1)
        ...
        if ifNodeAround:
            ...
            self.ax.set_xlim(coord[0]-400,coord[0]+400)
            self.ax.set_ylim(coord[1]-400,coord[1]+400)
        self.fig.savefig(os.path.join(foldername,"Figures", "Figure_%04d.png" % self.snapshotNumber), 
    ...
    def makeVideo(self, foldername = "Input", nameVideo = "Simul.avi"):
        listImagesUS = glob.glob( os.path.join(foldername, "Figures", "*png"))
        numSS_ar= np.zeros( len(listImagesUS) , dtype= np.int64)
    ...
    def deleteFigures(self, foldername = "Input"):
        figures = glob.glob( os.path.join(foldername,"Figures","*") ) 
        
        for f in figures:
            os.remove(f)
    
    def plotNetwork(self, labels=False, nodes=[]):
        colorNode = ["b","r"] 
        ...
        if len(nodes) > 0:
            plt.scatter(self.nodesdb[nodes,1], self.nodesdb[nodes,2], s=20, c="g")
        plt.axis("equal")
        plt.xticks([])
        plt.yticks([])
        plt.show()
    
    def plotBestChoices(self, foldername = "Input", ifUpdateLinkDensity= False, linkList=[], densityLvlList= [],
                        ifSaveFile= False, baseNameFile= "BC"):
        # self.denLvlArrPerLink[ind, :numComp]
        ...
        for i in range(self.linksdb.shape[0]):
            ...
            # plt.text(0.5*(self.nodesdb[int(self.linksdb[i,1]),1] + self.nodesdb[int(self.linksdb[i,2]),1]) , 0.5*(self.nodesdb[int(self.linksdb[i,1]),2] + self.nodesdb[int(self.linksdb[i,2]),2]), 
            #         "%d" % self.linksdb[i,0], fontsize=8, color='k')
        for i in range(self.nodesdb.shape[0]):
        ...
        if ifSaveFile:
            fileName= os.path.join(foldername,"Figures_BestChoice",baseNameFile + "_time%04d.png" % self.time )
